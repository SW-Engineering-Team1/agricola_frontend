...
export default {
  ...
  setup(props) {
    ...
    const stoneAccumulatedWest = ref(computed(() => store.state.accumulatedResources.stoneAccumulatedWest));
    ...
    const useWestQuarry = () => {
      if (props.round === 5) {
        socket.emit("useActionSpace", {
          ...
          goods : [
            { 
              name: "stoneAccumulatedWest",
              num: stoneAccumulatedWest.value,
              isAdd: true
            }
          ] 
        });
